...
class SchedulerService {
    static initTaskJobs(taskRepo, taskService) {
        return __awaiter(this, void 0, void 0, function* () {
            // 初始化所有启用定时任务的任务
            const tasks = yield taskRepo.find({ where: { enableCron: true } });
            tasks.forEach(task => {
            ...
            if (taskCheckCrons) {
                ...
                taskCheckCrons.forEach((cronExpression, index) => {
                    this.saveDefaultTaskJob(`任务定时检查-${index}`, cronExpression, () => __awaiter(this, void 0, void 0, function* () {
                        taskService.processAllTasks();
                    }));
                });
            }
            // 2. 重试任务检查 默认每分钟执行一次
            this.saveDefaultTaskJob('重试任务检查', '*/1 * * * *', () => __awaiter(this, void 0, void 0, function* () {
                yield taskService.processRetryTasks();
            }));
            // 3. 清空回收站 默认每8小时执行一次
            ...
            if (enableAutoClearRecycle || enableAutoClearFamilyRecycle) {
                this.saveDefaultTaskJob('自动清空回收站', ConfigService.getConfigValue('task.cleanRecycleCron'), () => __awaiter(this, void 0, void 0, function* () {
                    yield taskService.clearRecycleBin(enableAutoClearRecycle, enableAutoClearFamilyRecycle);
                }));
            }
            // 4. TV 剧集每日总集数刷新（自动跟踪连载剧集数变动，集数满足时自动完结）
            this.saveDefaultTaskJob('TV剧集总集数刷新', '0 2 * * *', () => __awaiter(this, void 0, void 0, function* () {
                const tasks = yield taskRepo.find({ where: { videoType: 'tv' } });
                const { TMDBService } = require('./tmdb');
                const { MessageUtil } = require('./message');
                const tmdbService = new TMDBService();
                const messageUtil = new MessageUtil();
                for (const task of tasks) {
                    if (task.status === 'completed' || !task.tmdbId)
                        continue;
                    try {
                        const detail = yield tmdbService.getTVDetails(task.tmdbId);
                        if (!(detail === null || detail === void 0 ? void 0 : detail.seasons))
                            continue;
                        const seasonNum = (() => {
                            var _a;
                            const n = task.shareFolderName || task.resourceName || '';
                            const m = n.match(/(?:Season|S|第)\s*(\d+)/i);
                            return m ? parseInt(m[1]) : ((_a = detail.lastEpisodeToAir) === null || _a === void 0 ? void 0 : _a.season_number) || null;
                        ...
                        if ((seasonData === null || seasonData === void 0 ? void 0 : seasonData.episode_count) && seasonData.episode_count !== task.totalEpisodes) {
            ...
            if (ConfigService.getConfigValue('task.enableAutoCheckin')) {
                this.saveDefaultTaskJob('每日云盘自动签到', ConfigService.getConfigValue('task.checkinCron'), () => __awaiter(this, void 0, void 0, function* () {
                    yield taskService.runDailyCheckin();
                }));
            }
            // 6. 账号Session心跳保活
            this.saveDefaultTaskJob('账号Session心跳保活', '0 */4 * * *', () => __awaiter(this, void 0, void 0, function* () {
                yield taskService.runAccountsKeepAlive();
            }));
        });
    }
    static saveTaskJob(task, taskService) {
        ...
        if (task.enableCron && task.cronExpression) {
            ...
            const job = cron.schedule(task.cronExpression, () => __awaiter(this, void 0, void 0, function* () {
                logTaskEvent(`================================`);
                logTaskEvent(`任务[${taskName}]自定义定时检查...`);
                // 重新获取最新的任务信息
                const latestTask = yield taskService.getTaskById(task.id);
                if (!latestTask) {
                ...
                if (latestTask.status === 'processing') {
                    ...
                    if (!processingStartTime || (now.getTime() - processingStartTime.getTime() > fiveMinutes)) {
                        ...
                        yield taskService.taskRepo.save(latestTask);
                    }
                    else {
                ...
                const result = yield taskService.processTask(latestTask);
                if (result) {
    ...
    static handleScheduleTasks(settings, taskService) {
        ...
        // 处理普通回收站任务
        handleScheduleTask(ConfigService.getConfigValue('task.enableAutoClearRecycle'), enableAutoClearRecycle || enableAutoClearFamilyRecycle, currentCron, settings.task.cleanRecycleCron, '自动清空回收站', () => __awaiter(this, void 0, void 0, function* () { return taskService.clearRecycleBin(enableAutoClearRecycle, enableAutoClearFamilyRecycle); }));
        // 处理每日自动签到任务
        handleScheduleTask(ConfigService.getConfigValue('task.enableAutoCheckin'), settings.task.enableAutoCheckin, ConfigService.getConfigValue('task.checkinCron'), settings.task.checkinCron, '每日云盘自动签到', () => __awaiter(this, void 0, void 0, function* () { return taskService.runDailyCheckin(); }));
        return true;
    }
}
SchedulerService.taskJobs = new Map();
SchedulerService.messageUtil = new MessageUtil();
