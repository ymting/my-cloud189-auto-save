...
class TmdbBackfillService {
     ...
     */
    async run() {
        if (this.isRunning) return { processed: 0, success: 0, failed: 0 };
        this.isRunning = true;
        ...
        try {
            ...
            for (let i = 0; i < toProcess.length; i += this.config.concurrency) {
                ...
                const results = await Promise.allSettled(
                    batch.map(item => this.fetchAndUpdateTmdb(item, taskRepo))
                );
     ...
     * 获取单个 TMDB 详情并更新相关任务
     */
    async fetchAndUpdateTmdb(item, taskRepo) {
        try {
    ...
    async triggerManually() {
        ...
        return {
            success: true,
            message: `补全完成：处理 ${result.processed} 个，成功 ${result.success} 个，失败 ${result.failed} 个`,
            ...result
        };
    }
